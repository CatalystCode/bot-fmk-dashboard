return {
	id: "ProxyBot_analytics_dashboard",
	name: "ProxyBot Analytics Dashboard",
	icon: "dashboard",
	url: "ProxyBot_analytics_dashboard",
  logo: "https://hellopegg.io/landing/images2/peggcharacterheader_slate.png",
	...
	config: {
		connections: {
			"application-insights": { appId: "b6391b18-83d8-4307-a54c-44035b758ff5",apiKey: "ipr4v4tley40s1j7k57donabjmm1acisdm2a8gz7" }
		},
		layout: {
			isDraggable: true,
			isResizable: true,
	...
	dataSources: [
		...
		},
		{
			...
			params: {
              ...
              | summarize country_count=count() by country |
              order by country_count`,
						mappings: { country: (val) => val || "unknown",country_count: (val) => val || 0 },
						calculated: (filterCountries, dependencies, prevState) => {

              if (!filterCountries) { return; }

              const countries = filterCountries.map((x) => x.country);
              let selectedValues = [];
              if(prevState['countries-selected'] !== undefined){
                selectedValues = prevState['countries-selected']
              }
              return{
                "countries-count": filterCountries,
                "countries-filters": countries,
                "countries-selected": selectedValues
              }
            }
					}
				}
		...
		},
		{
            ...
            query: (dependencies) => {
              ...
              return `
                where name == 'MBFEvent.UserMessage' |
                summarize count=count() by bin(timestamp, ${granularity}), name, country=tostring(client_CountryOrRegion) |
                order by timestamp asc `
            },
						mappings: { country: (val) => val || "unknown",count: (val) => val || 0 },
						filters: [{ dependency: "selectedCountries",queryProperty: "client_CountryOrRegion" }],
						calculated: (timeline, dependencies) => {
              ...
              timeline.forEach(row => {
                var { country, timestamp, count } = row;
                var timeValue = (new Date(timestamp)).getTime();

                if (!_timeline[timeValue]) _timeline[timeValue] = {
                  time: (new Date(timestamp)).toUTCString()
                };
                if (!_countries[country]) _countries[country] = {
                  name: country,
                  value: 0
                };

                _timeline[timeValue][country] = count;
                _countries[country].value += count;
              });

              var countries = Object.keys(_countries);
              var countryUsage = _.values(_countries);
              var timelineValues = _.map(_timeline, value => {
              ...
              return {
                "timeline-country-graphData": timelineValues,
                "timeline-country-Usage": countryUsage,
                "timeline-country-timeFormat": (timespan === "24 hours" ? 'hour' : 'date'),
	...
	filters: [
		...
		},
		{
			...
			icon: "textsms",
			dependencies: { selectedValues: "filters:countries-selected",values: "filters:countries-filters" },
			actions: { onChange: "filters:updateSelectedValues:countries-selected" },
			first: true
		}
	],
	elements: [
		...
		},
		{
			id: "countries-area",
			type: "Area",
			title: "Country Usage Timeline",
			subtitle: "Bot usage per country",
			size: { w: 4,h: 8 },
			dependencies: { values: "ai:timeline-country-graphData",lines: "ai:timeline-countries",timeFormat: "ai:timeline-country-timeFormat" },
			props: { isStacked: true,showLegend: false }
		},
    {
      id: "countriesbar",
      type: "BarData",
      title: "Country Usage Rate",
      subtitle: "Bot usage per country",
      size: { w: 4, h: 8 },
      dependencies: { values: "ai:countriesbar", bars: "ai:countries-bars"},
      props: { nameKey: "country" },
      actions: {
        onBarClick: { action: "dialog:intentsDialog", params: { title: "args:country", intent: "args:country", queryspan: "timespan:queryTimespan"} }
      }
    }
	],
	dialogs: [
